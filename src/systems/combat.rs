...
pub fn tower_building(
    ...
    let Ok(building_mode) = building_mode_query.single() else { return; };
    ...
    if let Some(cursor_position) = window.cursor_position() {
        if let Ok(world_position) = camera.viewport_to_world_2d(camera_transform, cursor_position) {
            ...
            let distance = player_transform.translation.distance(Vec3::new(world_position.x, 0.0, world_position.y));
        ...
        if tower.last_shot >= 1.0 {
            ...
            if let Some((enemy_pos_vec, enemy_entity)) = closest_enemy {
                if let Ok(mut enemy) = enemy_mut.get_mut(enemy_entity) {
                    ...
                    tower.last_shot = 0.0;
                    if enemy.health == 0 {
                        enemy_killed_events.write(EnemyKilled { position: enemy_pos_vec });
                    }
                }
            }
        }
